...
public class ApiDiscogsServiceImpl implements IApiDiscogsService {
	...
	@Override
	public List<AlbumWantlist> getAlbumsWantListFromReleases(List<Want> wants, User user){
		List<AlbumWantlist> albums = new ArrayList<AlbumWantlist>();
		for (Want want : wants) {
			AlbumWantlist albumWantlist = getAlbumWantListFromRelease(want);
			albumWantlist.setUser(user);
			albums.add(albumWantlist);
		}
		return albums;
	}
	/**
	 ...
	 * @return
	 */
	private AlbumWantlist getAlbumWantListFromRelease(Want want) {
		...
		if(infosAlbums!=null){
			...
			album.setAlbumName(infosAlbums.getTitle());
			albumWantList.setAlbumWantlistId(infosAlbums.getId());
			if(infosAlbums.getYear()!=null){
				album.setReleaseDate(String.valueOf(infosAlbums.getYear()));
			}
		}
		albumWantList.setDateAdded(want.getDateAdded());
		albumWantList.setAlbum(album);
		return albumWantList;
	}
